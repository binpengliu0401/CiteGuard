...
class PlannerActivityInput:
    ...
    already-resolved note candidates. The current Activity accepts only an empty
    note list, but the boundary is defined now so the later memory slice does not
    need to change the durable contract.
    """
    ...
    def __post_init__(self) -> None:
        ...
        # make failures immediate whether input comes from Temporal or local code.
        ...
        if not all(isinstance(note, ResearchNote) for note in self.existing_notes):
            raise TypeError("existing_notes must contain only ResearchNote objects")
...
class PlannerActivityOutput:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            ...
            raise TypeError("sub_questions must contain only SubQuestion objects")
